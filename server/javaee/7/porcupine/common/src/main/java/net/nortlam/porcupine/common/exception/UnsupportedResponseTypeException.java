...
public class UnsupportedResponseTypeException extends OAuth2Exception {
    ...
    private static final Logger LOG = Logger.getLogger(UnsupportedResponseTypeException.class.getName());

    public UnsupportedResponseTypeException(OAuth2 oauth) {
        super(oauth);
        // State: Always add the state if present (according to specification)
        if(oauth.hasState())
            addParameter(OAuth2.PARAMETER_STATE, oauth.getState());
        
        addParameter(OAuth2.PARAMETER_ERROR, OAuth2.PARAMETER_ERROR_UNSUPPORTED_RESPONSE_TYPE);
