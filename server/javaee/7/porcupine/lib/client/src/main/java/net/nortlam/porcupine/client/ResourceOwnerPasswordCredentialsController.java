...
public abstract class ResourceOwnerPasswordCredentialsController<T> 
                        extends AbstractPorcupineController<T> 
                                    implements Serializable, FetchResource<T> {

    private static final Logger LOG = Logger.getLogger(ResourceOwnerPasswordCredentialsController.class.getName());

    public ResourceOwnerPasswordCredentialsController() {
    }

    public ResourceOwnerPasswordCredentialsController(Class<T> typeParameterClass) {
        setTypeParameterClass(typeParameterClass);
    }

    @Override
    public abstract String getUsername(); // REQUIRED 
    @Override
    public abstract String getPassword(); // REQUIRED
    @Override
